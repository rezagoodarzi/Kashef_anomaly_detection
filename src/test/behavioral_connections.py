...
class Config:
    ...
    PELT_PENALTY: float = 4.0  # Higher = fewer change points
    MIN_SIGNAL_LENGTH: int = 5
    
    # Weights for composite score
    WEIGHT_PEARSON: float = 0.5
    WEIGHT_DIFF_CORR: float = 0.15
    WEIGHT_CHANGE_ALIGN: float = 0.35  # Higher weight - important for telecom
    WEIGHT_DIRECTION: float = 0
...
def analyze_sector_pair(
    ...
    direction_align = calculate_direction_alignment(signal1, signal2, cp1, cp2)
...
def build_sector_data(
    ...
    for sector in sectors:
        ...
        signal = pivot[sector].values
        
        # Detect change points
        change_points = detect_change_points_pelt(signal, config.PELT_PENALTY)
                    ...
                    if corr_sector in sector_data:
                        ...
                        for corr_cp in corr_cps:
                            if abs(corr_cp - cp_idx) <= 2:
                                corr_signal = sector_data[corr_sector]['signal']
