...
class SourceLocalizer:
    ...
    def localize(self, anomaly_sectors, correlated_neighbors):
        ...
        for _, sector in anomaly_sectors.iterrows():
            baseline_val = self.baselines.get(sector['NE'], {}).get(sector['hour'], {}).get('median', -110)
...
class AnomalyLocalizationPipeline:
    ...
    def process_data(self, data_df, neighbor_df):
        ...
        for idx, group in enumerate(anomaly_groups):
            ...
            # Localize source
            location = self.source_localizer.localize(group['sectors'], unique_neighbors)
            neighbors = self.neighbor_analyzer.find_correlated_neighbors(
            anomaly_sector['NE'],
            data_df,
            time_window,
            baseline=self.baselines.get(anomaly_sector['NE'])  # ADD THIS
            )
